...
excluded category, say what excludes it: either one of the three conditions in
\secref{c16:sec:crcfeed}'s rule for \code{crc_enable} (\code{txsr_bit_valid}, \code{txsr_stuff} and
\code{role}), or the fact that the field never passes through \code{tx_shift_reg} at all.
