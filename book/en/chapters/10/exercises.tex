% Section 10.10, from lectures/L10/appendix/intro_framing_exercises.md and
...
Exercise~\ref{c10:ex:package} reads the \code{can_def} package handed out and then works against it
at the keyboard.
\end{aside}
...
questions read the package, check that the derivations hold when the inputs change, and extend a copy
of it with values \secref{c10:sec:frameformat} named but the package does not yet hold.

\xpart{a} Read the \file{controller/can_def.vhd} handed out, with \secref{c10:sec:candef} beside it.
For each constant, say where in \secref{c10:sec:frameformat} it comes from, and split them into those
that are set and those computed from other constants. Which two of the computed ones would go
silently wrong, with nothing complaining at analysis, if they were typed in as numbers and someone
later changed the clock frequency?

The package is handed out in full rather than as a specification, and for two reasons worth
mentioning: a package contains no behaviour to design, so there is nothing between the values and
the code for you to work out, and the testbenches handed out read its names and types, so it is part
of the contract. What is new here is the VHDL itself, since a package declaration is a construct the
book has not used before. The names are the next eight chapters' vocabulary, and having read them
once, properly, is cheaper than looking them up eight times.

Do (e) and (f) in a scratch copy of the package, outside what becomes the group's repository: the
package handed out goes in unchanged.
...
(appendix~\ref{app:sim}), and then run \code{make build-project} from the root of the course
...
Now break something in the package deliberately, a missing semicolon or a misspelled type, rerun,
and read what GHDL says about it. Put it back, so that the file is exactly as it was handed out.
