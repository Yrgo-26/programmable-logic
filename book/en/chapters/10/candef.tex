...
Handed out, in \file{controller/can_def.vhd}, and presented on the whiteboard during the session.
The package is part of the contract and not something to design: six of the testbenches handed out
read its names and types directly, so it goes into the group's repository \textbf{unchanged}, just
like the testbenches. Every module you write from \chapref{c11:ch} onwards lands in the same
directory, and all of them except \code{meta_prev} read this one file.

The listings below are the package's contents, part by part, in the same order as in the file.
