...
\needspace{6\baselineskip}
\subsection*{Target}
The top level's entity should exist and analyse cleanly. After that it is up to the group: someone
may well start on \code{bit_timer} or \code{crc15} straight away. The four leaf modules,
\code{bit_timer}, \code{crc15}, \code{tx_shift_reg} and \code{rx_shift_reg}, do not depend on each
other and each has a testbench of its own, so a group of 4--5 can build them in parallel. It is the
one point in the project where the order genuinely matters to how fast you make progress; see
