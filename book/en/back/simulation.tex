...
per-session directories and nothing to copy between them; \file{can_def.vhd} in particular is written
\emph{once}, in \chapref{c10:ch}, and every later controller module reads that one file.
...
\begin{textdrawing}
controller/   can_def.vhd            ch 10, yours
              can_controller.vhd     ch 11, yours (ports only at first, grows through ch 17)
...
The two directories initially contain only the files handed out: eight testbenches, plus
\file{spi_slave.vhd} and \file{spi_def.vhd}. Every chapter from \chapref{c10:ch} adds a module, with
...
\file{bit_timer.vhd} still to do. \file{can_def.vhd} is not named as missing, since \chapref{c10:ch}
already wrote it.
